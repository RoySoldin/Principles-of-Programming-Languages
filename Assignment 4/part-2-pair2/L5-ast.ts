...
export const parseLitExp = (sexps: any[]): CExp | LitExp | Error =>{
    const pos = second(sexps).indexOf('.');
    if(pos !== -1){
        return parseCExp(['cons', second(sexps)[0],second(sexps)[2]]);
    }
    else return safeF(makeLitExp)(parseSExp(second(sexps)));
};
// TODO - check what ending
