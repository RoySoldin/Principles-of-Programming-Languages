...
{
    const constraint1 = checkEqualType(typeofExp(exp.val, tenv), exp.var.texp, exp);
    if (isError(constraint1))
        return constraint1;
    else
        return makeVoidTExp();
...
{
    return isEmpty(exp.exps)? Error("Empty Program") : evalTExpSeq(exp.exps, tenv);
};

// Purpose: compute the type of a sequence 
const evalTExpSeq = (exps: Exp[], tenv: TEnv): TExp | Error =>
{
    return isEmpty(rest(exps))? typeofExp(first(exps),tenv) :
    isDefineExp(first(exps)) ? evalDefineTExpsSeq(exps, tenv) :
    isError(typeofExp(first(exps), tenv))? typeofExp(first(exps), tenv):    //error handling
    evalTExpSeq(rest(exps), tenv);
}

const evalDefineTExpsSeq = (exps: Exp[], tenv: TEnv): TExp | Error => {
    const defineExp: DefineExp = first(exps);
    const defExpType= typeofDefine(defineExp, tenv);
    if(isError(defExpType))
        return defExpType;
    else
    {
        const extTEnv = makeExtendTEnv([defineExp.var.var],[defineExp.var.texp] , tenv);
        return evalTExpSeq(rest(exps), extTEnv); 
    }
};